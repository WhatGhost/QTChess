...
public:
    ...
    QVector<Step*> _steps;
    CtrlPanel* _cp;
    void setCtrlPanel(CtrlPanel *cp);
    /* game status */
    int _selectid;
    bool _bRedTurn;
    ...
    void killStone(int id,int isFake=0);
    void reliveStone(int id);
    void moveStone(int moveid, int row, int col);
    bool isDead(int id);
    QString getIconPath(int id);
    ...
    void saveStep(int moveid, int killid, int row, int col, QVector<Step*>& steps,int Fake=0);
    void backOne();
    void back(Step* step);
    virtual void back();
